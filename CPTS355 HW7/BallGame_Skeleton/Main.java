...
/*end of config arguments*/
	...
	private Ball redball, greenball, blueball;                      //Ball instance. You need to replace this with an array of balls.     
	...
	class HandleMouse extends MouseInputAdapter 
	{
    	...
    	public void mouseClicked(MouseEvent e) 
    	{
    		...
    		//Random rand3 = new Random(), rand4 = new Random();
				...
				if (nNode.getNodeType() == Node.ELEMENT_NODE) 
				{
					...
					if(eElement.getElementsByTagName("type").item(0).getTextContent().toString().equals("basicball"))
					{
						//redball= new Ball(
								...
								//);
						redball= new Ball(15, 400, 400, -1, -1, 0, 0, Color.red, outnoise, player,  gwindow);
						greenball= new Ball(15, 300, 350, 1, 1, 3, 1, Color.green, outnoise, player,  gwindow);
						blueball= new Ball(45, 500, 500, 1, -1, 0, 2, Color.blue, outnoise, player,  gwindow);
	...
	public void paint (Graphics g)
	{
		...
		/*if the game is over (i.e., the ball is out) display player's score*/
			...
			g.drawString("Final Score: " + player.getScore(), 130, 140);
			g.drawString ("Clicks: " + player.getClicks(), 650, 100);
			int totalhits = redball.hitcount + greenball.hitcount + blueball.hitcount;
			double accuracy = ((float)totalhits / (float)player.getClicks()) * 100.0;
			accuracy = (int)accuracy;
			g.drawString("Click Accuracy: " + accuracy + '%', 650, 140);
			g.drawString("The Most hit ball was: ", 650, 180);
			
			if (redball.hitcount >= greenball.hitcount && redball.hitcount >= blueball.hitcount)
			{
				g.setColor (Color.red);
				g.drawString ("Regular Ball: " + redball.hitcount, 650, 220);
			}
			else if (blueball.hitcount >= greenball.hitcount && blueball.hitcount >= redball.hitcount)
			{
				g.setColor (Color.blue);
				g.drawString ("Shrink Ball: " + blueball.hitcount, 650, 220);
			}
			else
			{
				g.setColor (Color.green);
				g.drawString ("Bounce Ball: " + greenball.hitcount, 650, 220);
			}
